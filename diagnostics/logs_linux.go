...
func FetchLogs(ctx context.Context, opts FetchOptions) ([]LogEntry, error) {
...
func getDmesg(ctx context.Context, count int, minLevel LogLevel) ([]LogEntry, error) {
...
func getJournal(ctx context.Context, count int, minLevel LogLevel) ([]LogEntry, error) {
...
func buildDmesgLevelFlag(min LogLevel) string {
	startIdx := 4
...
func parseDmesgFrom(r io.Reader, count int) ([]LogEntry, error) {
...
func parseDmesgLevel(level string) LogLevel {
	switch level {
	case "emerg":
...
func parseJournalFrom(r io.Reader) ([]LogEntry, error) {
	var entries []LogEntry
	scanner := bufio.NewScanner(r)
	var sourceBuilder strings.Builder
	var lastTimestamp int64 = 0
	...
	for scanner.Scan() {
		...
		if rawPriority, ok := jEntry["PRIORITY"]; ok {
			...
			if l, exists := priorityToLevel[priorityInt]; exists {
				level = l
			}
		}
...
func mapLogLevelToJournalPriority(l LogLevel) string {
	switch l {
	case LevelDebug:
		return "7"
	case LevelInfo:
