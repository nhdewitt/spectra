...
func TestCollect_BootTime(t *testing.T) {
	metrics, err := Collect(context.Background())
	if err != nil {
		t.Fatalf("Collect: %v", err)
	}
	...
	t.Logf("Collect BootTime:	%d", sm.BootTime)
...
func TestCollect_ProcessCount(t *testing.T) {
	metrics, err := Collect(context.Background())
	if err != nil {
		t.Fatalf("Collect: %v", err)
	}
	...
	t.Logf("Collect processes:		%d", sm.Processes)
...
func TestCollect_UserCount(t *testing.T) {
	metrics, err := Collect(context.Background())
	if err != nil {
		t.Fatalf("Collect: %v", err)
	}
	...
	t.Logf("Collect users:	%d", sm.Users)
