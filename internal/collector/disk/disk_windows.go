...
func CollectDisk(ctx context.Context) ([]protocol.Metric, error) {
	...
	for i := range 26 {
		...
		typeRet, _, _ := winapi.ProcGetDriveType.Call(uintptr(unsafe.Pointer(rootPathPtr)))
		driveType := uint32(typeRet)

		if driveType != winapi.DriveFixed && driveType != winapi.DriveRemovable {
			continue
		}
		...
		ret, _, _ := winapi.ProcGetVolumeInformation.Call(
		...
		ret, _, _ = winapi.ProcGetDiskFreeSpaceEx.Call(
			uintptr(unsafe.Pointer(rootPathPtr)),
