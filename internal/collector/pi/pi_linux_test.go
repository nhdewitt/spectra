...
func TestCollectClocks_NoVcgencmd(t *testing.T) {
...
func TestCollectVoltage_NoVcgencmd(t *testing.T) {
	if _, err := exec.LookPath("vcgencmd"); err == nil {
		t.Skip("vcgencmd is available, skipping test")
	}
...
func TestCollectClocks_Integration(t *testing.T) {
	...
	clocks, err := CollectClocks(ctx)
	if err != nil {
		t.Errorf("CollectClocks failed: %v", err)
	}
	if len(clocks) > 0 {
	...
	volts, err := CollectVoltage(ctx)
	if err != nil {
		t.Errorf("CollectVoltage failed: %v", err)
	}
	if len(volts) > 0 {
	...
	_, err = CollectThrottle(ctx)
	if err != nil {
		t.Errorf("CollectThrottle failed: %v", err)
	}

	gpu, err := CollectGPU(ctx)
	if err != nil {
		t.Errorf("CollectGPU failed: %v", err)
	}
	if len(gpu) > 0 {
...
func BenchmarkCollectClocks(b *testing.B) {
	if _, err := exec.LookPath("vcgencmd"); err != nil {
	...
	ctx := context.Background()
	for b.Loop() {
		_, _ = CollectClocks(ctx)
	}
}

func BenchmarkCollectVoltage(b *testing.B) {
	if _, err := exec.LookPath("vcgencmd"); err != nil {
		b.Skip("vcgencmd not available")
	}
	...
	ctx := context.Background()
	for b.Loop() {
		_, _ = CollectVoltage(ctx)
	}
}

func BenchmarkCollectThrottle(b *testing.B) {
	if _, err := exec.LookPath("vcgencmd"); err != nil {
		b.Skip("vcgencmd not available")
	}
