...
func CollectNetwork(ctx context.Context) ([]protocol.Metric, error) {
	...
	for iface, cur := range current {
		...
		metric := protocol.NetworkMetric{
			...
			MTU:       cur.MTU,
			Speed:     cur.Speed,
			RxBytes:   rate(delta(cur.RxBytes, prev.RxBytes), elapsed),
			RxPackets: rate(delta(cur.RxPackets, prev.RxPackets), elapsed),
			RxErrors:  rate(delta(cur.RxErrors, prev.RxErrors), elapsed),
			RxDrops:   rate(delta(cur.RxDrops, prev.RxDrops), elapsed),
			TxBytes:   rate(delta(cur.TxBytes, prev.TxBytes), elapsed),
			TxPackets: rate(delta(cur.TxPackets, prev.TxPackets), elapsed),
			TxErrors:  rate(delta(cur.TxErrors, prev.TxErrors), elapsed),
			TxDrops:   rate(delta(cur.TxDrops, prev.TxDrops), elapsed),
		}
