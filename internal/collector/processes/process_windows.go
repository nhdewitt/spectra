...
func Collect(ctx context.Context) ([]protocol.Metric, error) {
	...
	winapi.ProcGlobalMemoryStatusEx.Call(uintptr(unsafe.Pointer(&memStatus)))
	...
	for {
		...
		if err == nil {
			var memCounters winapi.ProcessMemoryCounters
			memCounters.CB = uint32(unsafe.Sizeof(memCounters))

			r1, _, _ := winapi.ProcGetProcessMemoryInfo.Call(
				uintptr(hProcess),
...
func getProcessSchedulerSummary() (map[uint32]schedSummary, error) {
	...
	for {
		ret, _, _ := winapi.ProcNtQuerySystemInformation.Call(
		...
		if ret == winapi.StatusInfoLengthMismatch {
			bufSize *= 2
			buf = make([]byte, bufSize)
			continue
		}
	...
	for {
		proc := (*winapi.SystemProcessInformation)(unsafe.Pointer(&buf[offset]))
		...
		for i := uint32(0); i < proc.NumberOfThreads; i++ {
			thread := (*winapi.SystemThreadInformation)(unsafe.Pointer(&buf[threadOffset]))
			s := winapi.ProcessState(thread.ThreadState)

			switch s {
			case winapi.StateRunning:
				running++
			case winapi.StateReady, winapi.StateStandby, winapi.StateDeferredReady:
				runnable++
			case winapi.StateWaiting, winapi.StateGateWaitObsolete, winapi.StateWaitingForProcessInSwap:
				waiting++
			default:
