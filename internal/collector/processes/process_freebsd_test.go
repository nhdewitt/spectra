...
// TestCollectes_Integration runs the full shared pipeline
// including delta math and normalizeProcState.
func TestCollectes_Integration(t *testing.T) {
	// First call: establishes baseline (no CPU deltas yet)
	metrics1, err := Collect(context.Background())
	if err != nil {
		t.Fatalf("first Collectes: %v", err)
	}
	if len(metrics1) != 1 {
	...
	metrics2, err := Collect(context.Background())
	if err != nil {
		t.Fatalf("second Collectes: %v", err)
	}
