...
func Collect(ctx context.Context) ([]protocol.Metric, error) {
	...
	ret, _, _ := winapi.WlanOpenHandle.Call(
		winapi.WlanApiVersion, // [in] DWORD dwClientVersion (2)
		0,                     // [in] PVOID (0)
	...
	defer winapi.WlanCloseHandle.Call(handle, 0)

	var ifaceList *winapi.WlanInterfaceInfoList
	ret, _, _ = winapi.WlanEnumInterfaces.Call(
		handle,                              // [in] HANDLE
		0,                                   // [in] PVOID
	...
	defer winapi.WlanFreeMemory.Call(uintptr(unsafe.Pointer(ifaceList)))
	...
	for i := range ifaces {
		...
		var dataSize uint32
		var connAttr *winapi.WlanConnectionAttributes
		var opcode uint32 = winapi.WlanIntfOpcodeCurrentConnection

		ret, _, _ := winapi.WlanQueryInterface.Call(
			handle, // [in] HANDLE
		...
		winapi.WlanFreeMemory.Call(uintptr(unsafe.Pointer(connAttr)))

		var rssiPtr *int32
		var rssi int
		opcode = winapi.WlanIntfOpcodeRssi
		...
		if ret == 0 && rssiPtr != nil {
			rssi = int(*rssiPtr)
			winapi.WlanFreeMemory.Call(uintptr(unsafe.Pointer(rssiPtr)))
		} else {
		...
		ret, _, _ = winapi.WlanQueryInterface.Call(
			handle,
			uintptr(unsafe.Pointer(&info.InterfaceGuid)),
			uintptr(winapi.WlanIntfOpcodeChannelFrequency),
			0,
			uintptr(unsafe.Pointer(&dataSize)),
		...
		if ret == 0 && freqPtr != nil {
			...
			winapi.WlanFreeMemory.Call(uintptr(unsafe.Pointer(freqPtr)))
		} else {
			var channelPtr *uint32
			ret, _, _ = winapi.WlanQueryInterface.Call(
				handle,
				uintptr(unsafe.Pointer(&info.InterfaceGuid)),
				uintptr(winapi.WlanIntfOpcodeChannelNumber),
				0,
				uintptr(unsafe.Pointer(&dataSize)),
			...
			if ret == 0 && channelPtr != nil {
				channel := *channelPtr
				winapi.WlanFreeMemory.Call(uintptr(unsafe.Pointer(channelPtr)))

				frequency = channelToFrequency(channel)
			}
		}

		results = append(results, protocol.WiFiMetric{
			Interface:   name,
			SSID:        ssid,
			SignalLevel: rssi,
			LinkQuality: quality,
...
func parseDot11SSID(ssid winapi.Dot11Ssid) string {
