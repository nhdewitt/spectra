...
func TestParseDot11SSID(t *testing.T) {
	tests := []struct {
		name string
		ssid winapi.Dot11Ssid
		want string
	}{
		{
			name: "Normal SSID",
			ssid: func() winapi.Dot11Ssid {
				s := winapi.Dot11Ssid{USSIDLength: 9}
		...
		{
			name: "Empty SSID",
			ssid: winapi.Dot11Ssid{USSIDLength: 0},
			want: "",
		},
		{
			name: "Max Length SSID",
			ssid: func() winapi.Dot11Ssid {
				s := winapi.Dot11Ssid{USSIDLength: 32}
		...
		},
		{
			...
			ssid: func() winapi.Dot11Ssid {
				s := winapi.Dot11Ssid{USSIDLength: 15}
		...
		},
		{
			...
			ssid: func() winapi.Dot11Ssid {
				s := winapi.Dot11Ssid{USSIDLength: 12}
...
func TestCollect_NoWiFiAdapter(t *testing.T) {
...
func TestCollect_ConsistentResults(t *testing.T) {
	ctx := context.Background()

	metrics1, err1 := Collect(ctx)
	if err1 != nil {
		t.Skip("WiFi not available")
	}

	metrics2, err2 := Collect(ctx)
	if err2 != nil {
...
func BenchmarkParseDot11SSID(b *testing.B) {
	ssid := winapi.Dot11Ssid{USSIDLength: 10}
...
func BenchmarkParseDot11SSID_MaxLength(b *testing.B) {
	ssid := winapi.Dot11Ssid{USSIDLength: 32}
