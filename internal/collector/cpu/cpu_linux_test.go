...
func TestCollect_Integration(t *testing.T) {
	...
	if metrics1 != nil {
		t.Error("Collect() first call should return nil")
	}
...
func TestCollect_StateReset(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("skipping: requires Linux /proc filesystem")
	}
...
func TestCollect_SequentialCalls(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("skipping: requires Linux /proc filesystem")
	}
	...
	for i := range 10 {
		...
		if metrics == nil {
			t.Fatalf("Collect() call %d returned nil", i)
		}
	}
}
...
func TestProcStatFromDeltas(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			usage := util.Percent(got.Used, got.Total)
