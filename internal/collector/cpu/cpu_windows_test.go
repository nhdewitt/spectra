...
func makeMockTimes(user, system, idle int64, count int) []winapi.SystemProcessorPerformanceInfo {
	stats := make([]winapi.SystemProcessorPerformanceInfo, count)

	for i := 0; i < count; i++ {
		stats[i] = winapi.SystemProcessorPerformanceInfo{
			UserTime: user,
...
func TestCollectWindows_StateManagement(t *testing.T) {
