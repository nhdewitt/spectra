...
var (
	...
	lastCPUTimes []winapi.SystemProcessorPerformanceInfo
...
// Collect collects CPU metrics for Windows.
...
func getSystemProcessorPerformanceInfo() ([]winapi.SystemProcessorPerformanceInfo, error) {
	...
	result := make([]winapi.SystemProcessorPerformanceInfo, numCores)

	bufferSize := uintptr(len(result)) * unsafe.Sizeof(result[0])
	var returnLength uint32

	ret, _, _ := winapi.ProcNtQuerySystemInformation.Call(
		uintptr(winapi.SystemProcessorPerformanceInformation),
...
func calculateCPUDeltas(current, last []winapi.SystemProcessorPerformanceInfo) (float64, []float64) {
...
func getProcessorCount() int {
	var info winapi.SystemInfo
	winapi.ProcGetNativeSystemInfo.Call(uintptr(unsafe.Pointer(&info)))
