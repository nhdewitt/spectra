...
func (s *Server) handleUpgradeInstructions(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("upgrade: parseAgentID err: %v", err)
	...
	if err != nil {
		log.Printf("upgrade: GetAgent err: %v", err)
		http.Error(w, "agent not found", http.StatusNotFound)
		return
	}

	log.Printf("upgrade: os=%q arch=%q valid_os=%v valid_arch=%v",
		agent.Os.String, agent.Arch.String, agent.Os.Valid, agent.Arch.Valid)

	p := platformFromAgent(agent.Os.String, agent.Arch.String)
	if p == nil {
		log.Printf("upgrade: no platform match for os=%q arch=%q", agent.Os.String, agent.Arch.String)
