...
func TestDecodeThrottle(t *testing.T) {
	tests := []struct {
		val   uint64
		check func(metrics.ThrottleMetric) bool
		name  string
	}{
		{
			val:  65537,
			name: "Undervoltage Now + History",
			check: func(m metrics.ThrottleMetric) bool {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m := result[0].(metrics.ThrottleMetric)
			if !tt.check(m) {
...
func TestCollectPi_Integration(t *testing.T) {
	...
	if len(clocks) > 0 {
		c := clocks[0].(metrics.ClockMetric)
	...
	if len(volts) > 0 {
		v := volts[0].(metrics.VoltageMetric)
